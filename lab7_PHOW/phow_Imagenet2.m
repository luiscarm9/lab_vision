...
for c=112:10:322
    %delete('baseline-hists.mat');
    %delete('baseline-vocab.mat');
tic
run('vlfeat-0.9.20/toolbox/vl_setup');
conf.calDir = '/datos1/vision/imagenet_small/train/';
conf.dataDir = '/datos1/vision/LuisRivera/lab_vision/lab7_PHOW/' ;
...
conf.modelPath = fullfile(conf.dataDir, [conf.prefix '-model' int2str(c) '.mat']) ;
conf.resultPath = fullfile(conf.dataDir, [conf.prefix '-result' int2str(c)]) ;
...
if ~exist(conf.vocabPath) || conf.clobber
  ...
  vocab = vl_kmeans(descrs, conf.numWords, 'verbose', 'algorithm', 'elkan', 'MaxNumIterations', 60) ;
...
if ~exist(conf.modelPath) || conf.clobber
  ...
  save(conf.modelPath, 'model','-v7.3') ;
else
  load(conf.modelPath) ;
end
...
subplot(1,2,2) ;
imagesc(confus) ;
accu=100 * mean(diag(confus)/conf.numTest);
title(sprintf('Confusion matrix (%.2f %% accuracy)', ...
              100 * mean(diag(confus)/conf.numTest) )) ;
print('-depsc2', [conf.resultPath '.ps']) ;
%save([conf.resultPath '.mat'], 'confus', 'conf') ;
delete(conf.histPath);
delete(conf.vocabPath);
toc

fprintf(file1,'%d %.3f %.3f\n',c,accu,toc);
